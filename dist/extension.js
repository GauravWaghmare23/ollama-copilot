...
// src/inlineProvider.ts
var InlineProvider = class {
  lastRequest = 0;
  async provideInlineCompletionItems(document2, position, _context, _token) {
    console.log("================================");
    console.log("MY INLINE PROVIDER TRIGGERED");
    console.log("FILE:", document2.fileName);
    console.log("LINE:", position.line);
    console.log("CHAR:", position.character);
    console.log("================================");
    const now = Date.now();
    if (now - this.lastRequest < 2e3) {
      console.log("RATE LIMITED");
      return { items: [] };
    }
    this.lastRequest = now;
    const contextText = document2.getText(
      new vscode.Range(
        new vscode.Position(
...
`);
      ...
      const completion = response.replace(/```[\s\S]*?```/g, "").replace(/```/g, "").trim();
...
${message.prompt}
`);
              this.lastResponse = this.extractCode(
                answer
              );
...
function activate(context) {
  ...
  console.log("================================");
  const inlineProvider = vscode3.languages.registerInlineCompletionItemProvider(
    [
      { language: "javascript" },
      { language: "typescript" },
...
${prompt}
`);
        const cleaned = extractCode(response);
        const editor = vscode3.window.activeTextEditor;
        if (editor) {
          await editor.insertSnippet(
            new vscode3.SnippetString(
              cleaned
            )
          );
          vscode3.window.showInformationMessage(
